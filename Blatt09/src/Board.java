...
public class Board {
     ...
     */
    public Board(String filename) { // liest nx und ny vom File und füllt field[][]
        Scanner in = null;
        try {
    ...
    public int manhattan() {
        // TODO 1.1 manhattan
        // Target ist für jeden Spielstein bekannt: (0,0) für 1, (1,0) für 2,...
        // berechne für jeden Stein Distanz zu Target
        // rechne alle Distanzen zusammen
        if(isSolved()){
            return 0;
        }
        int manhattanx = 0;
        int manhattany = 0;
        Position[] goalpos = new Position[nx*ny];

        for (int counter = 0; counter < nx * ny - 1; counter++) {
            goalpos[counter+1] = new Position(counter % nx, counter / nx);
        }
        for(int i = 0; i < positions.size(); i++){
            Position pos = positions.elementAt(i);
            int val = getField(pos);
            if(val != BLANK) {
                manhattanx += Math.abs(goalpos[val].x - pos.x);
                manhattany += Math.abs(goalpos[val].y - pos.y);
            }
        }
    ...
    public static void main(String[] args) {
        String filename = "src/board-manhattan1.txt";
